...
<div class="space-y-6">
    ...
    <div class="bw-card p-5 rounded-2xl flex flex-col xl:flex-row items-stretch xl:items-center justify-between gap-4">
        <form method="GET" action="{{ route('employees.index') }}" class="flex flex-col sm:flex-row items-center gap-2.5 w-full xl:w-auto flex-1">
            <!-- Search Input -->
            <div class="relative w-full sm:w-72">
                <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none text-slate-400">
                    <i data-lucide="search" class="w-4 h-4"></i>
                </div>
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Buscar nombre, ID o cédula..." class="bg-slate-100 border border-slate-300 text-slate-900 text-xs rounded-xl focus:ring-black focus:border-black block w-full pl-9 p-2.5 font-medium placeholder-slate-400">
            </div>

            <!-- Department Filter -->
            <select name="department_id" class="bg-slate-100 border border-slate-300 text-slate-900 text-xs rounded-xl focus:ring-black focus:border-black block w-full sm:w-56 p-2.5 font-semibold">
            ...
            <button type="submit" class="bg-black hover:bg-slate-800 text-white font-bold rounded-xl text-xs px-4 py-2.5 flex items-center justify-center w-full sm:w-auto shadow-sm">
                <i data-lucide="filter" class="w-3.5 h-3.5 mr-1.5"></i> Filtrar
            </button>
        </form>
