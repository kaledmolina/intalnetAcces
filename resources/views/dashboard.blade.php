...
<div class="space-y-6">
    ...
    <!-- Top Corporate Black & White Banner Card -->
    <div class="bw-card p-6 md:p-8 rounded-2xl bg-white border border-slate-200 shadow-sm relative overflow-hidden">
        <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 relative z-10">
            <div class="space-y-2 max-w-xl">
                <div class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-slate-100 text-slate-900 border border-slate-300">
                    <i data-lucide="shield-check" class="w-3.5 h-3.5 mr-1.5 text-slate-900"></i> Sistema de Control ISAPI
                </div>
                <h2 class="font-heading text-2xl md:text-3xl font-extrabold text-slate-900 tracking-tight">
                    Plataforma de Control de Asistencia
                </h2>
                <p class="text-xs md:text-sm text-slate-600 font-medium leading-relaxed">
                    Sincronización automatizada de registros dactilares y eventos de acceso con terminales biométricas Hikvision.
                </p>
            </div>

            <!-- Botones de Acción Corporativos en Blanco y Negro -->
            <div class="flex flex-wrap items-center gap-3 w-full md:w-auto">
                @if($devices->count() > 0)
                    <form action="{{ route('employees.import') }}" method="POST" class="w-full sm:w-auto">
                        @csrf
                        <button type="submit" class="w-full sm:w-auto text-slate-900 bg-white hover:bg-slate-100 border border-slate-300 font-extrabold rounded-xl text-xs px-4 py-2.5 flex items-center justify-center space-x-2 shadow-sm">
                            <i data-lucide="download-cloud" class="w-4 h-4 text-slate-900"></i>
                            <span>Importar Usuarios del Huellero</span>
                        </button>
                    </form>
                @endif

                <button type="button" data-modal-target="employeeModal" data-modal-toggle="employeeModal" class="w-full sm:w-auto text-white bg-black hover:bg-slate-800 font-extrabold rounded-xl text-xs px-4 py-2.5 flex items-center justify-center space-x-2 shadow-md">
                ...
                @if($devices->count() > 0)
                    <form action="{{ route('dashboard.sync') }}" method="POST" class="w-full sm:w-auto">
                        @csrf
                        <button type="submit" class="w-full sm:w-auto text-slate-900 bg-slate-100 hover:bg-slate-200 border border-slate-300 font-extrabold rounded-xl text-xs px-4 py-2.5 flex items-center justify-center space-x-2 shadow-sm">
                            <i data-lucide="refresh-cw" class="w-4 h-4 text-slate-900"></i>
                            <span>Sincronizar Marcaciones</span>
                        </button>
                    </form>
                @endif
            </div>
